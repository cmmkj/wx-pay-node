...
var WechatPayment = {
   ...
   *获取微信代扣签约接口
   */
  getEntrustwebInfoStr: function(param) {
    param.nonce_str = param.nonce_str || utils.createNonceStr();
    param.appid = WechatPayment.options.appid;
    param.mch_id = WechatPayment.options.mch_id;
    param.plan_id = WechatPayment.options.plan_id;
    param.version = param.version || '1.0'
    param.timestamp = utils.createTimestamp()
    param.sign = utils.sign(param, WechatPayment.options.apiKey);
    
    const { sign } = params
    const toEncodeParams = omit(params, ['sign'])
    return  `${utils.formatter(toEncodeParams, true)}&sign=${encodeURIComponent(sign)}`
  }
}
